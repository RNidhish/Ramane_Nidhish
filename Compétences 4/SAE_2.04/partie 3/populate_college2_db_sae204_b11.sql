...
-- Il manque des commune dans _commune
/*
insert into _commune (code_insee_de_la_commune, nom_de_la_commune, code_du_departement)
  values ('59271','Grande-Synthe','59');*/
...
insert into _quartier_prioritaire (code_quartier_prioritaire, nom_quartier_prioritaire)
  ...
  on CONFLICT(code_quartier_prioritaire) do nothing;
...
insert into _etablissement (uai, nom_etablissement,secteur, code_postal, code_insee_de_la_commune, nom_de_la_commune , code_academie, code_nature,lattitude, longitude)
  select distinct uai, nom_etablissement,statut_public_prive,code_postal, code_commune, nom_commune, code_academie , code_nature, latitude,longitude
  from _temp_fr_en_etablissements_ep
...
-- _est_a_proximite_de table okii

DELETE FROM _est_a_proximite_de;
INSERT INTO _est_a_proximite_de (code_quartier_prioritaire, uai)
SELECT DISTINCT e.qp_a_proximite, e.uai
FROM _temp_fr_en_etablissements_ep e
WHERE e.qp_a_proximite IS NOT NULL
  AND e.uai IS NOT NULL
  and exists(
  select 1
  from _quartier_prioritaire qp
  where qp.code_quartier_prioritaire = e.qp_a_proximite
  )
  and exists(
  select 1
  from _etablissement et
  where et.uai = e.uai
  );

ALTER TABLE _est_a_proximite_de
DROP CONSTRAINT IF EXISTS est_a_proximite_de_pk;

ALTER TABLE _est_a_proximite_de
ADD CONSTRAINT est_a_proximite_de_pk PRIMARY KEY (code_quartier_prioritaire, uai); 


-- caractéristiques tout etablissement
...
Where uai is not null
  ...
  and effectifs is not null;


-- Vue pourcentage filles

create or replace view vue_pourcentage_filles as
select 
    e.uai,
    e.nom_etablissement,
    e.code_postal,
    e.latitude,
    e.longitude,
    c.nombre_eleves_total - c.nombre_eleves_segpa - c.nombre_eleves_ulis as nombre_eleves_hors_segpa_ulis,
    (c._6eme_filles + c._5eme_filles + c._4eme_filles + c._3eme_filles) as total_filles,
    ((c._6eme_filles + c._5eme_filles + c._4eme_filles + c._3eme_filles)::decimal /
    (c.nombre_eleves_total - c.nombre_eleves_segpa - c.nombre_eleves_ulis)) * 100 as pourcentage_filles
from 
    _temp_fr_en_etablissements_ep e
join 
    _temp_fr_en_college_effectif_niveau_sexe_lv c on e.uai = c.numero_college
where 
    c.rentree_scolaire = '2022-2023';

-- Export de la vue vue_pourcentage_filles

WbExport -file=vue_pourcentage_filles.csv
         -outputDir=.
         -type=text
         -sourceTable=vue_pourcentage_filles
         -schema=colleges2
         -delimiter=';'
         -header=true
