...
void retirerCandidat(tCase2 *laCase, int valeur, int nbCandidatsElimines);
    ...
    for (i = 0; i < TAILLE; i += N)
    {
        for (j = 0; j < TAILLE; j += N)
        {
            ...
            for (k = 1; k <= TAILLE; k++)
            {
                ...
                if (count == 1)
                {
                    g[candidate_row][candidate_col].valeur = k;
                    retirerCandidat(&g[candidate_row][candidate_col], k, 0);
                }
            }
        }
    }
